...
Sends SMS through **OtimaTel**. This is the channel behind phone-number validation and password recovery codes, so it sits on the critical path of sign-up even though it runs asynchronously.

## Queue

Consumes `SMS_QUEUE` from RabbitMQ. Messages are processed with bounded retries and acknowledged only on success; failures are negatively acknowledged so nothing is lost silently.
